  ...
  return (
    <>
      ...
      <section style={{ paddingTop: '100px', paddingBottom: '72px', borderBottom: '1px solid var(--border)', position: 'relative', overflow: 'hidden' }}>
        <div style={{ position: 'absolute', inset: 0, background: 'radial-gradient(ellipse 60% 50% at 20% 60%, rgba(118,108,255,0.1) 0%, transparent 60%)', pointerEvents: 'none' }} />
        <div className="container" style={{ position: 'relative', zIndex: 1 }}>
          <p className="eyebrow">Our Work</p>
          <h1 style={{ ...F, fontSize: 'clamp(2.4rem,5vw,4rem)', fontWeight: 800, lineHeight: 1.0, letterSpacing: '-0.04em', color: '#fff', marginBottom: '16px', maxWidth: '700px' }}>
            Success Stories That Speak for Themselves
          </h1>
          <p style={{ fontSize: '17px', color: 'var(--text-2)', lineHeight: 1.8, maxWidth: '540px', marginBottom: '8px' }}>
                  ...
                  <Link key={String(item._id)} href={`/portfolio/${item.slug}`}
                    ...
                    <div style={{ padding: '26px', flex: 1, display: 'flex', flexDirection: 'column' }}>
                      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '12px' }}>
                        <span style={{ ...M, fontSize: '9px', textTransform: 'uppercase', letterSpacing: '0.1em', color, background: `${color}15`, border: `1px solid ${color}30`, padding: '3px 10px', borderRadius: '100px' }}>
                          {item.category}
                        </span>
                        {item.featured && (
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', gap: '4px', marginTop: '16px', ...M, fontSize: '11px', color }}>
      ...
      <section style={{ padding: '100px 0', borderBottom: '1px solid var(--border)', background: 'var(--bg-2)' }}>
        <div className="container" style={{ textAlign: 'center' }}>
          ...
          <h2 style={{ ...F, fontSize: 'clamp(2rem,4vw,3rem)', fontWeight: 800, letterSpacing: '-0.03em', color: '#fff', marginBottom: '16px' }}>
            Ready to be our next success story?
          </h2>
          <div style={{ display: 'flex', gap: '12px', justifyContent: 'center', flexWrap: 'wrap' }}>
            <Link href="/contact" className="btn btn-primary btn-xl">Start a Project <ArrowRight size={16} /></Link>
            <Link href="/contact" className="btn btn-outline btn-xl">Get Free Quote</Link>
          </div>
        </div>
      </section>
    </>
